...
option = ("hot_cold2 30 1 1" "hot_cold2 60 1 1" "hot_cold2 90 1 1")
#options=(
#
...
for option in "${options[@]}"; do
  echo "$option"
  timeout 30m /usr/bin/time -a -o run_all_bppy_bounded_output.csv -f "$option,%E,%M" python main.py $option
  EXIT_STATUS=$?
  if [ $EXIT_STATUS -eq 124 ]
  then
  echo "$option,t.o." >> run_all_bppy_bounded_output.csv
  else
  if [ $EXIT_STATUS -ne 0 ]
  then
  echo "$option,error,$EXIT_STATUS" >> run_all_bppy_bounded_output.csv
  fi
  fi
done